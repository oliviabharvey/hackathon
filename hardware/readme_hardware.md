...
*Electronic Warning: Make sure that you're are not using this setup on a metal surface. The electronic boards, like the Raspberry Pi, the homemade board, etc. are unshielded and could be shorted if placed on metal surface or if metal tools (like a screwdriver) is touching multiple points on the board.*
